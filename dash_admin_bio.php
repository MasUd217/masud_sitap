<?php
    ...
    //$adm = query ("SELECT * FROM tb_users WHERE id = $id")[0];
        ...
        if (ubah($_POST) > 0) {
            echo "
                <script>
                        ...
                        document.location.href = 'dash_admin_bio.php';
                </script>
            ";
        } else {
            echo "<script>
                        alert('Data Gagal diubah silahkan coba kembali!')
                        document.location.href = 'dash_admin_bio.php';
                </script>
            ";
        }
                      ...
                      <div class="card-body">
                        ...
                        <form action="" method="post">
                            ...
                            <div class="text-center">
                              <img src="asset/img/undraw_profile.svg" class="rounded-circle" alt="..." width="200px">
                            </div>
                            <div class="mb-3 text-dark">
                              <label for="nama" class="form-label">Nama Lengkap</label>
                              <input type="text" class="form-control" id="nama" name="nama" value="<?= $_SESSION["admin_name"]; ?>">
                            </div>
                            <!-- output tanggal pengguna -->
                            <div class="mb-3 text-dark">
                              <label for="tgl_lahir" class="form-label">Tanggal Lahir</label>
                              <input type="date" class="form-control" id="tgl_lahir" id="nama" name="tgl_lahir" value="<?= $_SESSION["admin_date"]; ?>" disabled>
                            </div>
                            <!-- output tempat lahir pengguna -->
                            <div class="mb-3 text-dark">
                              <label for="tempat_lhr" class="form-label">Tempat Lahir</label>
                              <input type="text" class="form-control" id="tempat_lhr" name="tempat_lhr" value="<?= $_SESSION["admin_birth"]; ?>" disabled>
                            </div>
                            <!-- output Jenis Kelamin -->
                            <div class="mb-3 text-dark">
                                <label class="form-label">Jenis Kelamin</label>
                                <select class="form-control" name="jk" disabled>
                                 <option value="<?= $_SESSION["admin_gender"]; ?>"><?= $_SESSION["admin_gender"]; ?></option>
                                 <option value="<?= $_SESSION["admin_gender"]; ?>"><?= $_SESSION["admin_gender"]; ?></option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3 text-dark">
                              ...
                              <input type="text" class="form-control" id="no_telp" name="no_telp" value="<?= $_SESSION["admin_phone"]; ?>">
                            </div>
                            <!-- output alamat pengguna -->
                            <div class="mb-3 text-dark">
                              <label for="alamat" class="form-label">Alamat</label>
                              <input type="text" class="form-control" id="alamat" name="alamat" value="<?= $_SESSION["admin_address"]; ?>">
                            </div>
                            <!-- tombol aksi -->
                            <div class="mb-3 mt-2">
                                <button type="submit"  name="update" class="btn btn-warning">Ubah Data</button>
